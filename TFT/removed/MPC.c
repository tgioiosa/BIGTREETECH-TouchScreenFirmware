...
static uint8_t curTool_index = TOOL0;
static MPC_TUNING mpcTuning = {DONE, NO_RESULT};
static MPC_PARAM mpcParameter[MAX_TOOL_COUNT] = {0};
...
bool hasMPC(void)
{
  return (mpcParameter[TOOL0].heater_power > 0);
}
    ...
    if (mpcTuning.status == DONE)
    {
      ...
      switch (key_num)
      {
        case KEY_ICON_4:
          do
          {
            curTool_index = (curTool_index + 1) % MAX_TOOL_COUNT;
