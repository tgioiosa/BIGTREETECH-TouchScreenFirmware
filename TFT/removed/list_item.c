...
/*  TG coMMented out it is not used
...
const char *const GET_ICONCHAR[CHARICON_NUM]={
...
GUI_POINT getTextStartPoint(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, ICON_ pos, const char * textchar)
{
...
void ListItem_Display(const GUI_RECT * rect, uint8_t position, const LISTITEM * curitem, bool pressed)
{
  ...
  if (position >= LISTITEM_PER_PAGE)
  {
    if (curitem->icon != ICONCHAR_BACKGROUND)
    {
      ...
      else if (curitem->icon == ICONCHAR_BLANK && curitem->titlelabel.index != LABEL_NULL)
      {
