...
void levelingProbePoint(LEVELING_POINT point)
{
  ...
  { // Marlin will respond to the G30 by sending a response "Bed X:n.nn Y:n.nn Z:-n.nn" and parseAck.c
    // here will place the Z value into var probedZ
    mustStoreCmd("G30 E1 X%d Y%d\n", coords[point].x_coord, coords[point].y_coord);  // move to selected point
  }

  probedPoint = LEVEL_NO_POINT;  // reset probedPoint before waiting for new probedZ to trigger
}

// this gets called from parseAck() when Marlin sends "Bed X: Y: Z:" with probeZ in response to G30
