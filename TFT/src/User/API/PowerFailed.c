...
bool powerFailedInitData(void)
{
  ...
  //TG commented out because Extrude.c was reoved for CNC
  //mustStoreCacheCmd("%s\n", tool_change[infoBreakPoint.tool]);

  for (uint8_t i = MAX_HEATER_COUNT - 1; i >= MAX_TOOL_COUNT; i--)  // Bed & Chamber infoCacheCmd.queue[0 - 1]
  {
...
void powerFailedCache(uint32_t offset)
{
  ...
  { // not paused, update printing progress status.
    ...
    for (uint8_t i = MAX_TOOL_COUNT; i < MAX_HEATER_COUNT; i++)  // Bed & Chamber
    {
      infoBreakPoint.target[i] = heatGetTargetTemp(i);
    }
