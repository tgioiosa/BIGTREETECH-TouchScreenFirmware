...
// save dynamic value (upto 7 digits) ( i : index of the value position, value:float value)  //TG version
void setDynamicValue(uint8_t i, float value)
{
  if (value < 1.0f)
...
void draw_itemtitle(GUI_POINT pos, LABEL label, uint8_t position, int textarea_width)
{
  if (label.index != LABEL_NULL)
  {
    ...
    if (label.index == LABEL_DYNAMIC)   // get the text to display from dynamic_label[5] array (it should be set upon loadFeatureSettings())
...
void ListItem_DisplayToggle(uint16_t sx, uint16_t sy, uint8_t iconchar_state)
{
  ...
  GUI_SetColor(charIconColor[CHARICON_TOGGLE_BODY]); //TG 2/27/23 changed to match the iconchar color from charicon.c  (was infoSettings.list_border_color);
...
void ListItem_DisplayCustomValue(const GUI_RECT * rect, LABEL value, int i)
{
  ...
  if (value.index == LABEL_CUSTOM_VALUE || value.index == LABEL_DYNAMIC)  // show custom text value
    GUI_DispStringInPrect(&rectVal, (uint8_t *)getDynamicTextValue(i));   // get from dynamic_text_value[5] array (it should be set upon loadFeatureSettings())
