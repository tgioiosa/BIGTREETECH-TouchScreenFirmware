                    ...
                    void (*prepareItem_action)(LISTITEM * item, uint16_t index, uint8_t itemPos))
{
  ...
  listViewSetCurPage(curPageIndex); // extract items form totalItems to create the current 5-item page (with <,>,back) for the current page index
  menuDrawListPage(&listItems);     // draw the current page
}
...
// display page at selected index, scans totalItems list to extract items for the current page index, each page is 5 items (8 with prev,next,back keys)
void listViewSetCurPage(uint8_t curPage)
{
  if (action_preparePage != NULL)                             // if any pre-action to prepare page is needed? (rarely specified, usually null)
  {
    action_preparePage(&listItems, curPage);
  }
  else
  {
    for (uint8_t i = 0; i < LISTITEM_PER_PAGE; i++)           // search and prepare up to 5 items for a single page
    {
      uint16_t index = curPage * LISTITEM_PER_PAGE + i;       // search starts at index 0,5,10,....etc. 

      if (index < maxItemCount)                               // maxItemCount is set by caller, this is the max number of items that can be searched
      {
        if (totalItems != NULL)                               // totalItems is a pointer to an ITEM list of all items to be searched (set by caller)
          listItems.items[i] = totalItems[index];             // listItems holds title and up to 8 items to be displayed for the current page

        if (action_prepareItem != NULL)
          action_prepareItem(&listItems.items[i], index, i);  // calls function pointer - usually set to loadFeatureSettings() to load value for display
      }
      else
      {
        listItems.items[i].icon = CHARICON_NULL;              // if ran out of totalItems before filling page, pad remainder with nulls
      }
    }
  }
...
// refresh whole list menu, call this after setting a submenu while in listview mode to restore listview
