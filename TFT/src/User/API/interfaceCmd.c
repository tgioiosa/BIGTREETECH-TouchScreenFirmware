...
#define CMD_QUEUE_SIZE 30 //TG 8/1/22 increased from 20
...
//TG 12/27/22 Added this useful routine which sends a gcode to the queue and then waits for Marlin to receive
//and finish executing the gcode. Marlin sends an "ok" after a gcode has completed processing. Not suitable for
//waiting on a Marlin response value string because the "ok/n" is parsed first and the value code may not get
...
////TG 2/16/23 Added this useful routine which sends a gcode to the queue and then waits for Marlin to receive
//// waits for the passed retmsg to be seen by ParseAck(). Returns 0=success or 1=timeout, not received.
//// Use this to ask Marlin for something and wait till it's been received by parseAck(). The retmsg should
//// be from the enumeration in 
//uint8_t msg_complete = 0;   // indicates when an 'M' g-code request message has finished
//uint8_t gcodeSendAndWaitForAnswer(msgcodes retmsg)
//{
//  uint16_t waitPeriod;
//  waitPeriod = isPrinting() ? 6000 : 4000;      // allow extra time for responses if printing
...
//  
//  while (msg_complete != retmsg)                // until we get the retmsg we expect
//  {
...
// If queue is full wait here until it opens up then send command.
  ...
  switch (cmd_ptr[cmd_base_index])
  {
    ...
    case 'M':
      switch (cmd_value())
      {
        ...
        //TG if WiFi port connected, handle these commands from WiFi connection and echo results back to WiFi connection:
        //   M20 (List SD Card), M23 (Select SD File), M24 (Start or Resume SD print), M25 (Pause SD print),
        //   M27 - Report SD print status, M28 - Start SD write, M29 - Stop SD write, M30 - Delete SD file, 
        //   M115 - TFT Firmware Info, M125 - Park Head, M524 - Abort SD print
        #ifdef SERIAL_PORT_2    
          case 20:  // M20
            if (!fromTFT)
            {
        ...
        case 306:  // M306
          //TG 2/24/23  MPC.c,h were removed for CNC
