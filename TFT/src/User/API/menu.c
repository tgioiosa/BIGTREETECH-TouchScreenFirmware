...
REMINDER reminder = {{0, 0, LCD_WIDTH, TITLE_END_Y}, 0, SYS_STATUS_DISCONNECTED, LABEL_UNCONNECTED};
...
void loopBusySignClear(void)
{
  ...
  if (menuType == MENU_TYPE_FULLSCREEN)
  {
    if (curMenuRedrawHandle != NULL) curMenuRedrawHandle();   //TG 1/28/23 added null test condition to prevent crashing
  }
  else
  {
...
void menuDrawTitle(void)
{
  ...
  //TG 10/4/22 - added to redraw WCS after a Toast msg drew over it on certain MENU's
  if (menuType == MENU_TYPE_ICON ||  MENU_TYPE_OTHER || menuType == MENU_TYPE_DIALOG) 
  {
...
if (item != NULL) {
  ...
  if (liveicon->enabled[linenum] == true)
  {
    ...
    setFontSize(liveicon->lines[linenum].font);
      ...
      case MENU_TYPE_ICON:
        {
          ...
          else if ((MENU_IS(menuSpindle)) ||
                   #ifdef USING_AVR_TRIAC_CONTROLLER
                   (MENU_IS(menuTriac)) ||           //TG 7/17/22 added so numpad will work
                   #endif
                   (MENU_IS(menuPrinting)) ||         //TG added so that infobox area can be used as a key
                   #ifndef NO_LOAD_UNLOAD  //TG 3/2/23 added global flag to exclude this code              //
                   (MENU_IS(menuLoadUnload)) ||
                   #endif
                   //(MENU_IS(menuMPC)) ||            //TG 2/24/23 removed MPC.c,h modules for CNC
                   //(MENU_IS(menuPid)) ||            //TG 2/14/21 removed Pid.c module for CNC
                   //(MENU_IS(menuTuneExtruder)) ||   //TG 2/10/21 removed TuneExtruder module for CNC
                   (MENU_IS(menuFan)) ||
                   //(MENU_IS(menuExtrude)) ||        //TG 2/8/21 removed Extrude module for CNC
                   (MENU_IS(menuSpeed)) ||
                   (MENU_IS(menuZOffset)) ||
...
void loopBackEnd(void)
{
  ...
  // Send TFT printing state
  loopPrintState();    //TG 2/15/23 added
  ...
  #ifdef SERIAL_PORT_2
    Serial_GetFromUART();   //TG this was parseRcvGcode() previously
  #endif
  // Temperature monitor
  loopCheckHeater();
  // Fan speed monitor
  ...
  {
    loopPrintFromOnboard();  // handle a print from (remote) onboard media, if any  was loopPrintFromHost()
  }

  #ifdef USB_FLASH_DRIVE_SUPPORT
    USB_LoopProcess();      // was USBH_Process()
  #endif

  #ifdef FIL_RUNOUT_PIN
    FIL_BE_CheckRunout();   // loop for back-end filant runout detection, was loopBackEndFILRunoutDetect()
  #endif
    ...
    #endif
    {
      LCD_Enc_CheckSteps(); // check change in encoder steps   was loopCheckEncoderSteps()
    }
  #endif

  #ifdef HAS_EMULATOR
    Mode_CheckSwitching();  // checks encoder button if user wants to switch lcd modes
  #endif
  ...
  #ifdef SMART_HOME
    loopCheckBackPress();   // check if Back is pressed and held
  #endif

  #ifdef LCD_LED_PWM_CHANNEL
    LCD_CheckDimming();     // checks if time to dim lcd, or restore brightness on key or encoder press
  #endif
...
void loopFrontEnd(void)
{
  ...
  #ifdef FIL_RUNOUT_PIN
    FIL_FE_CheckRunout(); // Loop for front-end filament runout detection
  #endif

  // loop for popup menu
  loopPopup();        //TG if popup_redraw is true this will handle the popup
}
