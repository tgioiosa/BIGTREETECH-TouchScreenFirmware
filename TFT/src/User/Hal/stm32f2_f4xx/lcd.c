...
//TG - Configure and Initialize GPIO pins for FSMC alternate configuration, these are
//     the data and control lines from the microcontroller to the LCD interface.
//     FSMC=Flexible Static Memory Controller peripheral in the STM32F207 microcontroller.
void LCD_GPIO_Config(void)
{
  //fsmc 16bit data pins
  GPIO_InitSet(PD0, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //FSMC_D2 pin 81
  GPIO_InitSet(PD1, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //FSMC_D3 pin 82
  GPIO_InitSet(PD8, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //FSMC_D13 pin 55
  GPIO_InitSet(PD9, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //FSMC_D14 pin 56
  GPIO_InitSet(PD10, MGPIO_MODE_AF_PP, GPIO_AF_FSMC); //FSMC_D15 pin 57
  GPIO_InitSet(PD14, MGPIO_MODE_AF_PP, GPIO_AF_FSMC); //FSMC_D0 pin 61
  GPIO_InitSet(PD15, MGPIO_MODE_AF_PP, GPIO_AF_FSMC); //FSMC_D1 pin 62
  GPIO_InitSet(PE7, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //FSMC_D4 pin 38
  GPIO_InitSet(PE8, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //FSMC_D5 pin 39
  GPIO_InitSet(PE9, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //FSMC_D6 pin 40
  GPIO_InitSet(PE10, MGPIO_MODE_AF_PP, GPIO_AF_FSMC); //FSMC_D7 pin 41
  GPIO_InitSet(PE11, MGPIO_MODE_AF_PP, GPIO_AF_FSMC); //FSMC_D8 pin 42
  GPIO_InitSet(PE12, MGPIO_MODE_AF_PP, GPIO_AF_FSMC); //FSMC_D9 pin 43
  GPIO_InitSet(PE13, MGPIO_MODE_AF_PP, GPIO_AF_FSMC); //FSMC_D10 pin 44
  GPIO_InitSet(PE14, MGPIO_MODE_AF_PP, GPIO_AF_FSMC); //FSMC_D11 pin 45
  GPIO_InitSet(PE15, MGPIO_MODE_AF_PP, GPIO_AF_FSMC); //FSMC_D12 pin 46
  ...
  */
  GPIO_InitSet(PD4, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //PD4 pin 85-FSMC_NOE :LCD-RD
  GPIO_InitSet(PD5, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //PD5 pin 86-FSMC_NWE :LCD-WR

#ifdef MKS_TFT35_V1_0  // different pinout for MKS_TFT35_V1_0
  GPIO_InitSet(PD12, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);
#else
  GPIO_InitSet(PE2, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //PE2 pin  1-FSMC_A23 :LCD-RS   LCD-RS data or cmd
#endif
  GPIO_InitSet(PD7, MGPIO_MODE_AF_PP, GPIO_AF_FSMC);  //PD7 pin 88-FSMC_NE1 :LCD-CS
}

//TG - Configure the FSMC peripheral for use with the LCD display
