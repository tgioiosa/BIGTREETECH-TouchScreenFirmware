...
void menuMachineSettings(void)
{
  ...
  KEY_VALUES curIndex = KEY_IDLE;
  //const ITEM itemCaseLight = {ICON_CASE_LIGHT, LABEL_CASE_LIGHT};   //TG 2/24/23 removed
  ...
  while (MENU_IS(menuMachineSettings))
  {
    ...
    switch (curIndex)
    {
      ...
      case KEY_ICON_3:
        #ifndef NO_LED_COLORS  //TG 3/2/23 added global flag to exclude this code  
      ...
      case KEY_ICON_6:
        //if (infoMachineSettings.caseLightsBrightness == ENABLED)    //TG 7/22/22 removed
        //  OPEN_MENU(menuCaseLight);            //TG 7/22/22 removed
        break;
